...
Reverse-engineered claude.ai web API client for TokenTrace: resolve `org_id` (cookie inspection then `/api/bootstrap` fallback), fetch usage from `/api/organizations/{org_id}/usage`, parse the three buckets (`five_hour`, `seven_day`, `seven_day_sonnet`), detect authentication failure across multiple signals (HTTP 401/403, redirect-to-login, HTML-when-JSON-expected), and persist the user-supplied session cookie in the macOS Keychain.
